  ...
  * @remarks Once a livestream session has started, the following events can be listened for:
  ...
  private async launchLivestream(cameraId: string, channel: number, options: LivestreamOptions): Promise<boolean> {
    ...
    try {
      ...
      this.log.debug(options.requestId + ": connecting to the livestream websocket API at %s", wsUrl);
      ...
      this.ws.addEventListener("open", () => {
        ...
        this.log.debug(options.requestId + ": connected to the livestream websocket API.");

        // Initialize our last message time.
